...
Item {
  ...
  readonly property int taskCount: getIntValue(pluginApi?.pluginSettings?.count, 0)
  readonly property int completedCount: getIntValue(pluginApi?.pluginSettings?.completedCount, 0)
  readonly property color contentColor: mouseArea.containsMouse ? Color.mOnHover : Color.mOnSurface
  readonly property string tooltipText: getProgressText()
  ...
  function getProgressText() {
    if (!pluginApi)
      return "";
    if (taskCount === 0)
      return pluginApi.tr("bar_widget.empty");
    return pluginApi.tr("bar_widget.progress").replace("{completed}", completedCount).replace("{total}", taskCount);
  }
  ...
  Rectangle {
    ...
    Row {
      ...
      NText {
        anchors.verticalCenter: parent.verticalCenter
        text: root.taskCount === 0 ? "0/0" : root.completedCount + "/" + root.taskCount
        color: root.contentColor
